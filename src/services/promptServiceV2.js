/**
 * Servicio de prompts mejorado v2 con formato de chat
 * Utiliza la estructura de roles (system, user, assistant) para mejor control conversacional
 */
...
const CONFIG = {
    ...
    timeout: 600000, // 5 minutos (aumentado para modelos grandes como qwen2.5:14b)
    temperature: 0.2,
    topP: 0.9,
    topK: 40,
    // Usar la API de chat para modelos que la soporten
    useChatAPI: true,
    // Timeout específico para intentar con chat antes de hacer fallback
    chatTimeout: 600000 // 1 minuto para chat, luego fallback a generate
};
...
/**
 * Convierte el formato antiguo al nuevo formato de chat
 * @param {Object} promptData - Datos del prompt en formato antiguo
 * @returns {Array} - Mensajes en formato de chat
 */
function convertToMessages(promptData) {
    const messages = [];
    ...
    if (promptData.conversationHistory && Array.isArray(promptData.conversationHistory)) {
...
/**
 * Consulta al modelo usando la API de chat
 * @param {Array} messages - Array de mensajes con roles
 * @param {Object} options - Opciones adicionales
 * @returns {Promise<string>} - Respuesta del modelo
 */
async function queryModelChat(messages, options = {}) {
    const maxRetries = options.maxRetries || CONFIG.maxRetries;
    
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            logger.debug(`Intento ${attempt}/${maxRetries} de consulta al modelo (chat API)`);
            ...
            logger.debug(`Enviando request a: ${process.env.OLLAMA_API_URL}/chat`);
            
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), CONFIG.chatTimeout || CONFIG.timeout);
...
/**
 * Fallback al método antiguo si la API de chat no está disponible
...
async function queryModelGenerate(promptData, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            logger.debug(`Intento ${attempt}/${maxRetries} de consulta al modelo (generate API - fallback)`);
...
/**
 * Consulta inteligente al modelo que intenta usar chat API primero
...
async function queryModel(promptData, options = {}) {
    try {
        // Si ya vienen mensajes en formato de chat, úsalos directamente
        if (Array.isArray(promptData)) {
            if (CONFIG.useChatAPI) {
                try {
                    return await queryModelChat(promptData, options);
                } catch (chatError) {
                    // Si falla por timeout o error, intentar con generate
                    if (chatError.name === 'AbortError' || chatError.message.includes('Timeout')) {
                        logger.warn('API de chat timeout, usando generate como fallback');
                        const systemMessage = promptData.find(m => m.role === 'system');
                        const userMessage = promptData.find(m => m.role === 'user');
                        return await queryModelGenerate({
        ...
        // Si vienen en formato antiguo, intentar convertir y usar chat API
        if (CONFIG.useChatAPI) {
            const messages = convertToMessages(promptData);
            try {
                return await queryModelChat(messages, options);
            } catch (chatError) {
                // Si falla por timeout o error, intentar con generate
                if (chatError.name === 'AbortError' || chatError.message.includes('Timeout')) {
                    logger.warn('API de chat timeout, usando generate como fallback');
        ...
        // Si falla la API de chat por 404, desactivar para futuras llamadas
        if (CONFIG.useChatAPI && error.message.includes('404')) {
            logger.warn('API de chat no disponible, desactivando para futuras llamadas');
            CONFIG.useChatAPI = false;
            return await queryModelGenerate(promptData, options);
        }
        throw error;
    }
...
/**
 * Detecta las intenciones usando el nuevo formato de chat
...
/**
 * Detecta intenciones con contexto conversacional usando formato de chat
 * @param {string} message - Mensaje del usuario
...
async function detectIntentionsWithContext(message, context, templateName = 'contextual-intent-detection', variables = {}, options = {}) {
    try {
        ...
        // Mensaje del sistema con instrucciones contextuales
...
/**
 * Genera una respuesta contextual usando formato de chat
...
// Mantener las funciones de extracción de entidades con el nuevo formato
...
async function testConnection() {
    try {
        ...
        const messages = [
            {
                role: "system",
                content: "Eres un asistente de prueba. Responde con 'OK' si recibes este mensaje."
            },
            {
                role: "user",
                content: "Test"
        ...
        const response = await queryModel(messages, { maxRetries: 1 });
...
module.exports = {
    ...
    testConnection,
    getModelInfo,
    // Exportar configuración para modificación externa si es necesario
    CONFIG
};